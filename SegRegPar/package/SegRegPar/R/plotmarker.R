...
cond.col=NULL, cond.col.sample=NULL, cond.prefix="day", 
yname="normalized expression",par.param = c(1,1),
pdfheight=15, pdfwidth=10,...){
		data.norm <- data
		...
		if(seg){
		if(is.null(fittedres))tmp <- fit.seg(data,listname[i],t.vect = t.vect,maxk = min(round(length(unique(t.vect))/5,0),10),min.num.in.seg = 3,cutdiff = .01, ...)
