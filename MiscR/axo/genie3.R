...
get.weight.matrix <- function(expr.matrix, K="sqrt", nb.trees=1000, input.idx=NULL, importance.measure="IncNodePurity", seed=NULL, trace=TRUE, ...) {
    ...
    weightList <- foreach(target.gene.idx=seq(from=1, to=num.genes))%dopar%{
    #for (target.gene.idx in seq(from=1, to=num.genes)) {
        ...
        list(im.names,target.gene.name, im)
    }
#    print(weightList)
    for(i in 1:length(weightList)){
      weight.matrix[weightList[[i]][[1]],weightList[[i]][[2]]] <- weightList[[i]][[3]]
    }
