...
srE <-  lapply(names(experiments),function(n){
  ...
  if(!file.exists(file.path(datapath,paste0(n,"Events.txt")))){
    ...
    m.k <- 
    SegRegEvents(set1 = list(experiments[[n]]), tp1 = list(tps[[n]]*24),min.num.in.seg =m.k ,maxk = min(round(length(unique(tps[[n]]))/5,digits = 0),10),output.dir = datapath, fn=paste0(n,"k",m.k,"Events"),logT = T) 
  }
})

round(.015/min(diff(unique(tpDatasets[[29]]))))

a <- read.table("~/code/data/cb/NBNogData/segregevents/MouseMegaEndodermEvents.txt")
a$gene <- toupper(a$gene)

plot(datasets[[27]]["LFNG",])
...
outputs <-  lapply(names(experiments),function(n){
  ...
  a.tfs <- a.sig
  #a.tfs <- a.tfs[a.sig$gene%in%intersect(outputs[[8]]$gene, outputs[[10]]$gene),]
  ...
  pdf(paste0('~/Desktop/',n,"ALL",'.pdf'),height = 10,width = 20)
  x1 <- ggplot(dat) +
...
inds <- order(abs(outputs[[10]]$fc),decreasing = T)[5:10]
...
std.heatmap(cor(datasets[[27]],method = "pearson"))
...
a <- read.table("~/code/data/cb/NBNogData/segregevents/NBNogHiResRobotMouseEvents.txt")
b <- read.table("~/code/data/cb/NBNogData/segregevents/NBNogHiResRobotHumanEvents.txt")
a <- readRDS("~/code/data/cb/NBNogData/segregevents/HiResRobotMouseEvents.rds")
a <- readRDS("~/code/data/cb/NBNogData/segregevents/NBNogHiResRobotHumanEvents.rds")

a.sig <- a[a$slp.pval<.05 & abs(a$fc)>1,]
b.sig <- b[b$slp.pval<.05 & abs(b$fc)>1,]

hist(a.sig$slp,breaks = seq(-200,200,.5),xlim=c(-15,15), col=rgb(1,0,0,0.5) )
hist(b.sig$slp,breaks = seq(-200,200,.5),add=T,col=rgb(0,0,1,0.5))
legend("topleft",legend = c("mouse","human"),fill = c(rgb(1,0,0,0.5),rgb(0,0,1,0.5)))

ggplot(melt(as.data.frame(cbind("mouse"=a.sig$slp , "human"=b.sig$slp))))+
  geom_histogram(aes( value,color=variable))+
  scale_x_continuous(limits = c(-20, 20))
  
  geom_point(aes(Slope,value,color=variable))+
  

hist(a$slp,breaks = seq(-200,200,1),xlim = c(-10,10))
hist(b$slp,breaks = seq(-200,200,1),xlim = c(-10,10))
...
x <- round.log(datasets[[25]][iii,])
y <- round.log(datasets[[27]][iii,])

q <- iii[rank(-rowMads(x),na.last = T)<100 & rank(-rowMads(y),na.last = T)<100]

plot(y["WNT3",])

x[iii[order(rowSds(x),decreasing = T,na.last = T)<3000],]



compareTS <- function(a,b,pval.thresh =.25, fc.thresh=2){
